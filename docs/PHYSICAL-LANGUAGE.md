...
считаются физически рискованными; placement отклоняет пятый. Piston angular DOF
намеренно мягкие (Jolt angular springs) — длинные стеки слегка пружинят, как в
Space Engineers. Крупный вариант — `piston_base_large` / `piston_head_large`
(`specs/POC-ACTUATORS-V1.md`). Для дверей и декора допустим `kinematic` solver,
...
`passenger_seat` — только anchor посадки, без руля):
