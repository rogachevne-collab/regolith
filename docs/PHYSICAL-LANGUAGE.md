...
- *Структурные* (редкие, обязаны быть надёжными и упорядоченными): `attach`,
  `detach`, `weld`, `break`, `repair`, `connect_network`, `disconnect_network`.
...
2. Electric Network: батарея → привод.
3. Cargo Flow: бур → бункер.
