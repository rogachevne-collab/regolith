...
`specs/ROVER-MODULES-V1.md`. Там колесо и подвеска — два элемента: подвеска несёт
raycast-модель хода, колесо крепится к её `wheel_socket` обычным `Rigid` joint,
а assembly с готовой парой колесо+подвеска компилируется в dynamic `RigidBody3D`.
