...
| A4 | `SuitState` — Node на сцене игрока, вне `SimulationWorld` | `suit_state.gd` | не реплицируется, не сохраняется per-peer |
...
`SuitState` (health/oxygen/hydrogen) сейчас тикает локально на клиенте.
v0: оставляем тик на клиенте, но:

- клиент шлёт хосту своё состояние 2 Гц (для HUD союзников и для сейва);
- смерть — команда к хосту, хост подтверждает и решает респавн (иначе
  два клиента могут разойтись в том, кто жив).

Полный перенос `SuitState` в `SimulationWorld` — задача v1, но он бы
снял A4 целиком и его стоит рассмотреть до начала работ.
...
Миграция `save_version 1 → 2`: старый `"player"` становится
`players["<host_uid>"]`, стор `"player"` → `"player:<host_uid>"`.
...
| 1 | `player_uid`, `store_id` per-peer, убрать дефолты `"player"`, `is_player_store()`, миграция сейва v1→v2 | `run_one.sh test_industry_v1`, `test_store_snapshot` зелёные, одиночная игра не сломана |
