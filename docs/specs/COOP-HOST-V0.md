...
Статус: этапы 0–3 реализованы и играбельны (host/join, видно друг друга,
строят вместе через снапшот-ребродкаст); этапы 4–7 не начаты. См. «Что уже
работает» за код-указателями и известными отклонениями от спеки. Цель —
кооп на 2–4 игрока: один игрок = хост (listen server), остальные
подключаются, сессия и сейв живут у хоста.
...
| A5 | Правки террейна применяются локально и никуда не публикуются | `terrain_excavation_service.gd`, `terrain_impact_carver.gd` | клиент не увидит выкопанное | не сделано (этап 4); клиент временно не может копать вообще (блок-лист) |
| A6 | Позы сборок живут только в Jolt на хосте | `simulation_physics_projection.gd`, `assembly_motion_state.gd` | клиент не увидит движение | не сделано (этап 6); сборки долетают только со снапшот-ребродкастом, между ними позы у клиента заморожены |
...
`voxel_remove`, `dig_terrain_debris`, `scoop_spoil`, `dump_scoop`,
`debug_spawn_spoil`, `place_block`, `toggle_control_seat` — гейтвей отвечает
`not_in_coop_yet` вместо попытки и десинка. Копка/бур и кресла — версия для
этапов 4 и 7 соответственно.
...
Этапы 0–2 не требовали сети и были полезны сами по себе. Этап 3 пройден —
**точка остановки «продолжаем или нет» позади**, кооп играбелен для
пробного забега (без копки, без езды на ровере вместе, без кресел).
...
| **R-COOP-7** Хост не даёт клиенту `VoxelViewer` — стример хоста не расширяется вокруг чужих игроков | низкая для триала (игроки рядом) | реализовать per-peer viewer, когда террейн начнёт реплицироваться (этап 4) и клиенты смогут разбредаться |
