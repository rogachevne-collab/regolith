...
| 6 | Physics replication (позы сборок, f64) | частично (спайк C: `_cli_assembly_motion` 15 Гц, без f64-пакета спеки) | ровер едет — обоим видно гладко |
| 7 | Кресло: `seated`, ввод водителя, выход | частично (спайк C: occupancy + `toggle_control_seat` + `_srv_control_input`; без Passenger Seat / ходьбы на платформе) | гость садится и ведёт; аватар в кресле |
