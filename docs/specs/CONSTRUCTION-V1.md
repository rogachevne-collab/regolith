...
Terrain anchors — только для стационарных построек: **mobile (locomotive)
assembly никогда не получает anchor joint** от установленного блока, даже
если блок касается грунта, и исключена из terrain-anchor reconcile — блок на
...
|---|---:|---:|
| `frame`, `frame_basalt`, `cargo_pipe`, `drive_wheel` | 1×1×1 | 0.5×0.5×0.5 m envelope |
| `frame_slope_45` | 1×1×1 | 0.5³ m wedge (visual 45°; box collider v1) |
| `frame_antenna` | 1×4×1 | 0.5×2.0×0.5 m mast (grey visual) |
| `frame_lamp` | 1×1×1 | headlight; pivot = mount; mesh `Lamp` = emissive lens |
...
2. **Face scan** — stateless марш aim-луча по occupancy grid каждой
   attach-allowed Assembly (kernel-индекс, кэш по topology revision):
   посещённые клетки + magnet-пробы соседей (Manhattan ≤ 2) дают exposed
   faces возле луча; вход луча в занятую клетку даёт entry face и обрывает
   марш (сквозь стену не магнитится). Scoring: ray deviation, distance от
   `ray_origin`, screen deviation, authoritative validity.
3. **Voxel fallback** — валидный `KIND_VOXEL` direct hit получает
   ...
   (общий пул кандидатов, отбор по score). Ground plan ленивый: пока валидная
   magnetic face выигрывает, план земли не считается (seat raycasts, большие
   footprints — зря); manual cycle добирает его в пул.
...
**Performance:** face scan — **stateless**: никакого world-space face cache,
инвалидаций, bucket-индекса и pose sync нет вообще. Источник истины — kernel
occupancy index (кэш по `(assembly_id, topology_revision)`) и **текущие**
transforms: припаркованный ровер магнитится, уехавший перестаёт — без
structural events и без staleness по построению. Дальние Assembly отсекаются
revision-кэшированным local AABB (slab test) до скана; `construction_attach_allowed`
(joint scan) проверяется только для Assembly, до которых луч дотянулся.
Клетки на driven body groups (выдвинутый piston carriage) в root-space скане
пропускаются — их покрывает direct aim. Plan validation ленивая: первый
валидный кандидат + sticky (hysteresis), остальные ranked faces не
валидируются; верхняя граница попыток — top-K. Reuse результата — один слой:
quantized context key в `ConstructionPreview` поверх
`SimulationWorld.topology_generation` + resolve heartbeat 150 ms (velocity-flip
парковки не даёт structural event). Preview mesh nodes строятся один раз на
(archetype, orientation, valid) в origin ZERO и переносятся transform'ом
...
TOP-K face prefilter — GDScript `.has` по revision-кэшу occupancy (без
переpacking всего rover в C++ на каждый ranked face). Attach validate —
один native `validate_attach_preview` на flat Packed* буферах (occupancy +
assembly element/face tables кэш по `(assembly_id, topology_revision)`,
rotated footprint + connections + bridge cycle in-process). Magnet snapshot —
topology POD (packed occupancy / origins / local AABB) revision-кэш; в
snapshot попадают только assembly, чьи AABB пересекает aim ray (тот же cull,
что раньше жил только в C++ после полной сборки). `single_group` — минимальный
element dict (root pose); multi-group — live transforms + driven-path idle
одним `compile_body_groups` на assembly. TOP-K prefilter probes capped
(`TOP_K_PREFILTER`). Driven-head idle check остаётся в GDScript после
connections. Без Dictionary side-spec на hot path.
...
prefilter, `validate_attach_preview`, body-group compile math, split/merge
momentum) may run in C++ when the library is present; GDScript remains
authority for resource/archetype/revision gates, `ConstructionPlacement.plan()`,
structural mutate commands, and scene-tree projection. Missing DLL → identical
GDScript fallbacks. Snapshot for native magnet scan is built by
`ConstructionPreviewSnapshot` (POD occupancy + transforms; no `SimulationWorld`
pointer in C++).
