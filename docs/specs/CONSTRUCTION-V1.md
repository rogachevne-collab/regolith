...
anchored assemblies в local space (+ cached world_point). Full/incremental rebuild
face topology — только при смене assembly set / topology revision / structural
events. Continuous pose sync **пропускается** для frozen/terrain-anchored
assemblies (иначе walk+miss-aim → десятки ms/frame на rebucket). Mobile assemblies
обновляют world_point из local pose при сдвиге > half-cell, без bump `generation`.
Miss-aim quantize грубее (preview). Каждый resolve: corridor pass + top-K
`ConstructionPlacement.plan()`. Gateway/Preview reuse при неизменном quantized
aim/selection/cache generation.
