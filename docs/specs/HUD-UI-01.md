...
как `SimulationSuitState`, по одному на `player_id` — чтобы оно сохранялось
и реплицировалось общим механизмом снапшота. Узел `SuitState` остался на
сцене игрока как **view**: он подписан на `SimulationWorld.suit_changed`,
переизлучает `changed` и форвардит `*_fraction()`. Для Vitals контракт ниже
