...
Не числовые уставки, а переключатели: в фейсплейте рисуются **строкой-тумблером**
...
### Параметры (уставки) — установить / шаг
...
Один каталог обслуживает **и** строки «Уставки» в фейсплейте, **и** привязываемые
...
- каталог глаголов из таблицы выше для piston/rotor/hinge/wheel; исполнение
  и хоткеем клавиши, и кликом по командной кнопке в фейсплейте;
...
- `param.set/increase/decrease` из ParameterCatalog (`game_balance.json`),
  включая уставки подвески; `±` в строке уставки бьёт той же командой;
- навигатор: сегмент-фильтр, поиск, выбор по `element_id` (переживает
  пересборку списка 10 Гц), клик по строке аварии открывает узел;
- переименование узла (`set_element_name`) из шапки фейсплейта;
- бар 9 страниц × 9 клавиш, drag-drop привязки, ПКМ снимает клавишу,
