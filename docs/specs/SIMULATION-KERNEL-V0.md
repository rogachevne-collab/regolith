...
Construction v1 lethal `DamageElementCommand` (`integrity → 0`) — topology mutation:
элемент удаляется через общий removal/split path с `dismantle`, но
`refund_fraction = 0`. Persistent `broken` элемента в snapshot/topology нет.
Подробности — `docs/specs/CONSTRUCTION-V1.md`.
