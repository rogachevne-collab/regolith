...
##
## Off, and left here for what trying it established rather than for the knob.
##
## It has a consequence beyond the picture: the mesher feeds collision too, so
## whatever a floor stops drawing also stops holding the player up — measured,
## a floor of 0.06 already dropped a thin skirt the stand's probe had been
## standing on. And it did not buy what it was for. The torn edge is not cells
## flickering on a threshold that a floor can raise them clear of; it is what a
## quarter-metre isosurface does at the boundary of a sparse field, and no
## setting of this moves that.
const RENDER_MIN_FILL := 0.0
...
func _refresh_shell_cell(index: int) -> void:
	...
	var facing := _faces_open(x, y, z)
	if mass < GranularGrainShell.MIN_CELL_MASS or (
		not facing and not _backs_open(x, y, z)
	):
		_animating.erase(index)
...
func _faces_open(x: int, y: int, z: int) -> bool:
	return (
		_is_open(x + 1, y, z)
		or _is_open(x - 1, y, z)
		or _is_open(x, y + 1, z)
...
func _is_open(x: int, y: int, z: int) -> bool:
	return (
		region.field.mass_at(x, y, z) < OPEN_MIN_MASS
...
func _flush_box(dirty_box: AABB) -> void:
	...
	# The kernel needs a ring of input beyond every cell it writes, so it works
	# on a box grown by its own radius with the written box sitting inside.
	var work_extent := extent + Vector3i.ONE * (SMOOTH_RADIUS * 2)
	_reconstruct(lo - Vector3i.ONE * SMOOTH_RADIUS, work_extent)
	_paste_reconstructed(lo, extent, work_extent)
...
## Turn the reconstructed field into signed distance and hand the box over.
func _paste_reconstructed(
	lo: Vector3i,
	extent: Vector3i,
	...
	for y in extent.y:
		for z in extent.z:
			...
			for x in extent.x:
				...
				wi += 1
				vi += extent.y * 2
	buffer.set_channel_from_byte_array(VoxelBuffer.CHANNEL_SDF, _sdf_bytes)
	_tool.paste(lo, buffer, 1 << VoxelBuffer.CHANNEL_SDF)


func flush_report() -> String:
	return "%d cells / %d chunks %.2f ms   %s" % [
		_last_flush_cells,
		_last_flush_chunks,
		_last_flush_ms,
