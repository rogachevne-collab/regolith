...
func _test_lethal_damage_removes_element_without_refund() -> bool:
	...
	var element_id := int(spawn.data["element_ids"][0])
	var element := world.get_element(element_id)
	var store_before := world.get_resource_store(PlayerIdentity.store_id("player")).amount(
		"plate_metal"
	)
	...
	var store_after := world.get_resource_store(PlayerIdentity.store_id("player")).amount(
		"plate_metal"
	)
...
func _test_lethal_damage_refunds_with_fraction() -> bool:
	...
	var refunded := world.get_resource_store(PlayerIdentity.store_id("player")).amount("plate_metal")
