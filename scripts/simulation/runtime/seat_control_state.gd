...
## ActionBarState. Wheels+gyros on by default; thrusters off so Space is a
## pure service brake on stock rovers (hybrid Space→brake+lift is opt-in).
...
var control_thrusters: bool = false
...
static func from_dict(data: Dictionary) -> SeatControlState:
	...
	state.control_thrusters = bool(data.get("control_thrusters", false))
