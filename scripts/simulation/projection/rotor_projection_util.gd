...
const MIN_TOP_INERTIA_KG_M2 := 0.001
...
static func top_inertia_about_axis(
	head_body: PhysicsBody3D,
	axis_world: Vector3
) -> float:
	if not head_body is RigidBody3D:
		return MIN_TOP_INERTIA_KG_M2
	var axis := axis_world.normalized()
	var inverse_inertia: Basis = (
		(head_body as RigidBody3D).get_inverse_inertia_tensor()
	)
	var inverse_about_axis := axis.dot(inverse_inertia * axis)
	if inverse_about_axis <= 0.000001:
		return MIN_TOP_INERTIA_KG_M2
	return maxf(1.0 / inverse_about_axis, MIN_TOP_INERTIA_KG_M2)
...
static func effective_desired_angular_velocity_rad_s(
	motor: SimulationMotorState,
	top_inertia_kg_m2: float
) -> float:
	var commanded := desired_angular_velocity_rad_s(motor)
	if commanded == 0.0:
		return 0.0
	var effective_inertia := maxf(top_inertia_kg_m2, MIN_TOP_INERTIA_KG_M2)
	var denom := motor.damping_n_s_per_m + (
		effective_inertia / maxf(VELOCITY_RESPONSE_TIME_S, 0.0001)
	)
	if denom <= 0.0001:
		return commanded
	var velocity_cap := motor.force_limit_n / denom
	return signf(commanded) * minf(absf(commanded), velocity_cap)
...
static func compute_motor_torque_scalar(
	...
	if motor.control_mode != SimulationMotorState.ControlMode.STOP:
		desired_velocity = effective_desired_angular_velocity_rad_s(
			motor,
			top_inertia_kg_m2
		)
