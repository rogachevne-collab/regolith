...
| Glow | Screen blend (4.6+ default order: before tonemap), intensity 0.1, threshold 4.0, levels 3–5 (tight) | Vacuum has no atmospheric bloom, but the camera does. Threshold kept high so only the sun disc / hot emitters bloom — not the sunlit ground. Visible sun is `SolarSkyDecor` mesh (not sky-shader soap). Was Additive; switched to Screen to match Godot 4.6+ glow-before-tonemap behaviour. |
